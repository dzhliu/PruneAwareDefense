...
femnist_ec_dataset = None
femnist_edge_test_loader = None
...
def poison_data_with_edgecase_trigger(data, target, poison_frac = 0.2):
    ...
    target = copy.deepcopy(target)
    poison_number = math.floor(len(target) * poison_frac)
    random.shuffle(femnist_ec_dataset)
    for index in range(poison_number):
        target[index] = 1
        data[index]= femnist_ec_dataset[index]
...
def test_mali_edge_case(temp_model):
    ...
    correctly_labeled_samples = 0
    temp_model.eval()
    for batch_idx, (data, target) in enumerate(femnist_edge_test_loader):
